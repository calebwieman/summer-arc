...
function bgForCount(count: number): string {
  if (count === 0) return "bg-transparent border border-border";
  if (count <= 3) return "bg-accent/20 border border-accent/30";
  if (count <= 6) return "bg-accent/50 border border-accent/60";
  return "bg-accent border border-accent";
}

function textForCount(count: number): string {
  if (count >= 7) return "text-[#0a0a0a]";
            ...
            <motion.button
              ...
              className={`aspect-square flex items-center justify-center rounded-lg text-[13px] tabular-nums transition-colors ${
                disabled
                  ...
                  : `${bgForCount(count)} ${textForCount(count)} ${
